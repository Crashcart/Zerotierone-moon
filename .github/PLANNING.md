...
- [2026-05-26] Research audit: corrected DDNS bug — ZT_PUBLIC_ENDPOINT was documented as accepting DDNS hostname but stableEndpoints requires bare IP. Fixed .env.example, install.sh prompt, README Step 7, RESEARCH.md. Also corrected RESEARCH.md buffer size (25 MB → 8 MB). PLANNING.md + TODO.md brought current.
- [2026-05-26] AI-rules v1.28.1 loaded (prev v1.25.1). New rules: RULE 21 upstream sync check; RULE 22 one-way rule flow from canonical AI-rules repo; RULE 23 plan persistence (plans must exist in repo as plans/active/{slug}.md). RULE 23 compliance: created plans/active/zerotier-moon-maintenance.md.
