...
- [2026-05-20] AI-rules v1.9.0 imported. New active rules: COMPANY IDENTITY (production-ready code, excellent programmers); RULE 15 COMPLIANCE ENFORCEMENT (one correction limit, fix immediately, no explanations); RULE 16 HIRING APPROVAL (role must exist in agents/ OR have explicit user approval before use). Approved roster: 30 roles — see crashcart/AI-rules/agents/. Self-correction: subagent announced as "DEVELOPER" (not a roster role); correct role is FULLSTACK DEVELOPER or BACKEND DEVELOPER.
