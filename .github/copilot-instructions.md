...
| 13 | ✅ **ALWAYS use branch-specific install scripts** | Each branch has its own installer (`install-<branch>.sh`); `main` uses `install.sh`. README.md must reference the correct installer filename for the current branch. See `.github/BRANCH_AWARE_FILES.md`. |
