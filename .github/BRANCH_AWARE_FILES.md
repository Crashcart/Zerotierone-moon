...
These files contain content that **must match the current branch name**. When code is promoted between branches (`alpha → beta`, `beta → main`), an agent must update every entry below so install URLs and references point to the correct branch.
...
| `beta` | Pre-release testing | `/beta/install.sh` |
